...
namespace cadmium::example::auction {
	...
	struct AuctioneerState {
		...
		AuctioneerState(): time_to_calculate(), new_value(false), send_out(false), send_allocation(false) {
		} 
	};
	...
	std::ostream& operator<<(std::ostream& out, const AuctioneerState& au) {
		out << "{" << au.time_to_calculate <<"," << au.new_value <<"," << au.send_out <<"," << au.send_allocation <<",";
	...
	class Auctioneer : public Atomic<AuctioneerState> { //! Atomic models MUST inherit from the cadmium::Atomic<S> class
	 ...
	 public:
		 ...
		 */
		Auctioneer(const std::string& id, std::vector<std::vector<int>> _matrix, float _round_timer): Atomic<AuctioneerState>(id, AuctioneerState()), matrix(_matrix), round_timer(_round_timer) {
		...
		void output(const AuctioneerState& au) const override {
			if (au.send_out == true)
			{
				...
				if(au.send_allocation == true){
					allocation->addMessage(au.send_allocation);
				}
			}
		...
		[[nodiscard]] double timeAdvance(const AuctioneerState& au) const override {
			...
			{
				return 0;
			} else if (au.send_allocation == true){
